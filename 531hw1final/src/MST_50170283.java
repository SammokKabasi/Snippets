...
public class MST_50170283 {
	...
	public void prim() {
		...
		while (graph.getVertices().size() > graph.getVisitedVertices().getVertexList().size()) {
			...
			for (Vertex visitedVertex : graph.getVisitedVertices().getVertexList()) {
				...
				if (!graph.getVisitedVertices().getVertexList().contains(minKeyValuePair.getVertex())) {
					...
					// graph.getVertices().get(kvp.getVertex().getVertexId() -
					// 1).getEdges().delete(visitedVertex.getVertexId());
			...
			try {
				...
				// graph.getVertices().get(b.getVertexId() -
				// 1).getEdges().delete(a.getVertexId());
			...
			System.out.println(graph.getVisitedVertices().getVertexList());
		...
		for (int i = 0; i < mst.getVertices().size(); i++) {
			for (int j = 0; j < mst.getVertices().get(i).getEdges().getKeyValuePairs().size(); j++) {
				String string = mst.getVertices().get(i).getVertexId() + " " + mst.getVertices().get(i).getEdges().getKeyValuePairs().get(j).getVertex().getVertexId() + " " + mst.getVertices().get(i).getEdges().getKeyValuePairs().get(j).getWeight()
...
class MinHeapImpl {
	...
	public static void main(String[] args) {
		...
		heap.insert(new KeyValuePair(10, vertex));
		heap.insert(new KeyValuePair(1, new Vertex()));
		heap.insert(new KeyValuePair(5, new Vertex()));
		heap.insert(new KeyValuePair(3, new Vertex()));
		heap.insert(new KeyValuePair(2, new Vertex()));
	}
	...
	public void delete(int vertexId) {
		...
		heapifyDown(j);
		// System.out.println("AFTER DELETE " + vertexId + " -> " +
		// keyValuePairs);
	}
...
class MinHeapForVertices {
	...
	public void insert(Vertex vertex) {
		...
		try {
			...
			System.out.println("AFTER HEAP INSERT->" + vertexList);
		}
	}
	...
	public static void main(String[] args) {
		...
		MinHeapForVertices vertices = new MinHeapForVertices();
		vertices.insert(new Vertex(10));
		vertices.insert(new Vertex(1));
		vertices.insert(new Vertex(5));
		vertices.insert(new Vertex(3));
		vertices.insert(new Vertex(2));
		System.out.println(vertices.getVertexList());
