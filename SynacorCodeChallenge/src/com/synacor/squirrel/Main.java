...
public class Main {
	...
	public void run() {
		...
		if (input.trim().equalsIgnoreCase("1")) {
			...
			if (stealList.size()>0) {
				System.out.println("However, you may steal an acorn of this type, but you have to fight! Would you like to fight the owner of this acorn? (yes/no)");
				if(acceptInput().trim().toUpperCase().equalsIgnoreCase("YES")) {
					System.out.println("Unsupported operation.... :( Ran out of time");
				} else {
					System.out.println("No acorns for you, squirrel.");
				}
			}
